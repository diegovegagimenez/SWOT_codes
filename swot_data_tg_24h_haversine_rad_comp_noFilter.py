...
dmedia = np.arange(5, 110, 5)

# Loop through all netCDF files in the folder
nc_files = [f for f in os.listdir(folder_path) if f.endswith('.nc')]

# List to store all results for each radius
...
for rad in dmedia:
    ...
    results_rad_comparison.append({'radius': rad, 'rmsd': table['rmsd'].mean(), 'n_tg_used': len(table.dropna(subset=['rmsd']))})
    print(f'results for radius {rad} km added to the list')

results_df = pd.DataFrame(results_rad_comparison)
results_df

plt.plot(results_df['radius'], results_df['rmsd'])
plt.xlabel("Radius size (km)")
plt.ylabel('RMSD (SWOT - TG)')
plt.xticks(np.arange(min(results_df['radius']), max(results_df['radius'])+1, 5), rotation = 45)
plt.grid(True, alpha = 0.2)