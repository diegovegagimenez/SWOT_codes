...
# Paths to data
swot_path = '/home/dvega/anaconda3/work/SWOT_STORM/df_SWOT_v1_0_2_series_EUR_5km.csv'
TG_path = '/home/dvega/anaconda3/work/SWOT_STORM/datos_tg_cmems/TG_CMEMS_eur/'
...
def process_station(station):
    try:
        ...
        if swot_ts.empty:
            ...
            tg_ts = tg_ts.drop_dims(['POSITION', 'LATITUDE', 'LONGITUDE'])
            tg_ts = tg_ts.to_dataframe()

            # Reset the index to make 'TIME' a column and drop 'DEPTH'
            tg_ts = tg_ts.reset_index(level=['TIME', 'DEPTH'])  # Resets both 'TIME' and 'DEPTH' from index to columns
            tg_ts['TIME'] = pd.to_datetime(tg_ts['TIME'])
